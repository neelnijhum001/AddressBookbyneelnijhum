...
void namefun(){
	...
	char ch;
	gotoxy(31,4);
	printf("Do you wanna add more datas.Press n for that:");
	Sleep(1000);
	fflush(stdin);
	while((ch=getch())=='n'){
		menu();
...
void searchfun(){
	...
	while(fscanf(fptr,"%s %s %s %s %lf\n",name,address,gender,gmail,&phone)!=EOF){
		...
		Sleep(1000);
			gotoxy(18,12);
			printf("Enter n for menu option.");
	while(getch()=='n'){
		menu();
	}
	}
	...
	if(flag==0){
		...
		gotoxy(39,6);
		printf("Enter a to enter file again or double n key to open menu section:");
		if(getch()=='a'){
			system("cls");
			searchfun();
		}
...
void listfun(){
	...
	while(fscanf(fptr,"%s %s %s %s %lf",name,address,gender,gmail,&phone)!=EOF){
		...
		Sleep(1000);
		printf("Enter n for menu section:");
		while(getch()=='n'){
			menu();
		}
			fclose(fptr);
...
void modifyfun(){
	...
	fclose(fptr);
	fclose(fptr1);
	fptr=fopen("nijhum.txt","w");
	fclose(fptr);
	fptr=fopen("nijhum.txt","a");
...
void exitfun(){
	system("cls");
	gotoxy(31,4);
	printf("\xDB\xDB\xDB\xDB DEVELOPED BY \xDB\xDB\xDB\xDB");
	gotoxy(31,6);
	printf("\xDB NEELOTPAL NIJHUM");
	gotoxy(31,8);
	printf("\xDB CSE 41 Batch");
	gotoxy(31,10);
	printf("\xDB Metropoliton University,Sylhet.");

}
void password(void){
	char passwords[20]={"nijhum123"};
	gotoxy(22,2);
	int j;
	int z;
	char name[70]="A PERSONAL ADDRESSBOOK by Neelotpal Nijhum";
	z=strlen(name);
	for(j=0;j<=16;j++){
		Sleep(15);
		printf("\xDB");
	...
	gotoxy(30,7);
	printf("Are you Neelotpal Nijhum??? then log in with your password:");
	char ch,pass[20];
	char w='*';
	int i=0;
	while(ch!=13){
	...
	if(strcmp(pass,passwords)==0){
		gotoxy(30,9);
		printf("HELLO Neelotpal Nijhum you are LOGGED IN SUCCESSFULLY");
		Sleep(2700);
		menu();
	}
	else{
		gotoxy(30,9);
		printf("You entered the wrong password.Can't access to the system");
		Sleep(1500);
		system("cls");
		password();
}
